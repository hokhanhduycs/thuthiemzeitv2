...
let cameraImage: HTMLCanvasElement
...
export default class MiniMapScene extends MiniMapItem {
    ...
    onclick: ((p: MiniMapPoint) => void) | null = null
    onchangeselect: ((p: Array<MiniMapPoint>) => void) | null = null
    ...
    constructor(element: HTMLElement, opt?: MRenderConfig) {
        super()
        el = element
    ...
    loaddata(indata:any) {
        ...
        if (data.gps) {
            gpsMatrix.compose(
                new Vector3((data.gps.east + data.gps.west) / 2, (data.gps.north + data.gps.south) / 2, 0),
                new Quaternion().setFromEuler(new Euler(0, 0, data.gps.rotation * Math.PI / 180)),
                new Vector3(data.gps.east - data.gps.west, data.gps.north - data.gps.south, 1)
            )
        }
        this.pos = data.pos
        this.rot = data.rot
    ...
    private onPointerWheel(e: WheelEvent) {
    ...
    private addSelect(ls: Array<MiniMapItem>) {
        for (const item of ls) {
            item.select = true
        }
    }
    private removeSelect(ls: Array<MiniMapItem>) {
        for (const item of ls) {
            item.select = false
        }
    }
    ...
    private onPointerUp(e: PointerEvent) {
        ...
        if (mousemode == MOUSE_MODE.Left && new Date().getTime() - tmptime < 500) {
            ...
            if (selectedArea.width < 10 && selectedArea.height < 10) {
                let l = 1000000
                let nearest: MiniMapPoint | null = null
                this.traverse(item => {
                    if (item instanceof MiniMapPoint) {
    ...
    lockItem(obj: MiniMapItem, isLock: boolean) {
        obj.lock = isLock
        obj.traverse(item => {
            item.lock = isLock
        }, true)
    ...
    setSelectProfile(profile: string) {
        this.traverse(item => {
            if (item instanceof MiniMapPoint && item.select) {
    ...
    getSelectValue() {
        ...
        const p = new MiniMapPoint(all[0]) as any
        for (const item of all) {
            for (const key in p) {
                if (typeof p[key] == 'string' && p[key] != (item as any)[key])
                    p[key] = '-'
                if (typeof p[key] == 'number' && p[key] != (item as any)[key])
                    p[key] = '-'
            }
        }
        return p
    ...
    async export(onlySelect?: boolean) {
        const exportdata = {
            reject: {
                data: <any>[]
            }
    ...
    updateCam(pos: Vector3, rot: number) {
        if (!ready) {
            campos.x = pos.x
            campos.y = pos.y
            camrot = rot
    ...
    private genCam() {
        ...
        if (MRenderConfig.config.camera) {
            ...
            img.onload = () => {
                ...
                const camctx = cameraImage.getContext('2d') as CanvasRenderingContext2D
