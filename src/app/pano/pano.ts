import {
    ...
    Quaternion, 
    Matrix4, 
    PlaneGeometry, MeshBasicMaterial, Mesh, Frustum, 
    PerspectiveCamera, 
    Texture, 
    TextureLoader, 
    Object3D,
    SphereGeometry, DoubleSide, Vector3, Vector2,
    NearestFilter, LinearMipmapNearestFilter, LinearFilter,
...
export default class Pano {
    ...
    setMaxAnisotropy(val: number) {
        maxAnisotropy = val
    }
    setPanoType(panotype: number, quality: number) {
    ...
    loadPano(data: any) {
        ...
        this.obj.traverse(tile => {
            if(tile instanceof Mesh){
                const mat = tile.material as MeshBasicMaterial
                if (mat.map) {
                    mat.map.dispose()
                }
                tile.visible = false
    ...
    generateCubeRing(count: number, angle: number, basecount: number): Group {
        const obj = new Group();
    ...
    createSplitPlane(group: Group, face: number) {
    ...
    createSphereForZoom(tilesRequired: number): Group {
        const group = new Group();
        const sx = (1 + 100 / (99 + this.quality)) * 2
        group.scale.set(sx, sx, sx)
    ...
    createSphereMeshForTile(group: Group, tilesRequired: number, tileX: number, tileY: number) {
    ...
    async loadHighTexture(obj: Object3D) {
    ...
    loadTexture(url: string): Promise<Texture> {
        return new Promise<Texture>((rel, rej) => {
